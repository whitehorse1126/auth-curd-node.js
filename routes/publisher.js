...
const SQL_SELECT_ALL =
    "SELECT * FROM dbo.Publisher ORDER BY PublisherId ASC for json path;";

// for json path, without_array_wrapper - use for single json result
const SQL_SELECT_BY_ID =
    "SELECT * FROM dbo.Publisher WHERE PublisherId = @id for json path, without_array_wrapper;";

const SQL_INSERT =
    "INSERT INTO dbo.Publisher (PublisherName) VALUES (@PublisherName); SELECT * from dbo.Publisher WHERE PublisherId = SCOPE_IDENTITY(); ";

// GET listing of all Publisher
// Address http://server:port/Publisher
// returns JSON
...
// Address http://server:port/Publisher/:id
// returns JSON
...
router.post(
    ...
    async(req, res) => {
        ...
        const PublisherName = validator.escape(req.body.PublisherName);
        if (PublisherName === "") {
            errors += "invalid PublisherName; ";
        }
